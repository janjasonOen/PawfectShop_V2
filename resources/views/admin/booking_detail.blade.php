...
<div class="container py-4">
    <h3 class="mb-1">Booking #{{ (int)$bookingId }}</h3>
    <div class="text-muted mb-3">Detail booking (service) dan status.</div>
        ...
        <div class="col-lg-7">
            <div class="card shadow-sm">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start gap-3">
                        <div>
                            ...
                            @if(!empty($booking->booking_date))
                                <div class="small text-muted mt-2">Tanggal: {{ $booking->booking_date }}</div>
                            @endif
                        </div>
                    ...
                    <div class="mt-2" style="white-space:pre-line;">{{ $booking->customer_address ?? '' }}</div>
                    ...
                    <div class="small text-muted">
                        ...
                        @if($paymentProofPath !== '')
                            ...
                            @if(!empty($booking->payment_proof_uploaded_at))
                                <span class="text-muted">({{ $booking->payment_proof_uploaded_at }})</span>
                            @endif
                        @else
                    ...
                    <form method="POST" action="{{ route('admin.booking_detail', ['id' => (int)$bookingId]) }}" class="d-flex gap-2" style="max-width:420px;">
                        @csrf
                        <select name="status" class="form-select" required>
                            @foreach($validStatuses as $s)
                                <option value="{{ $s }}" @selected($s === strtolower((string)($booking->status ?? 'pending')))>
                                    {{ ucfirst($s) }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @foreach($items as $it)
                            ...
                            <div class="d-flex justify-content-between align-items-start gap-3 py-2 border-top">
                                <div>
                                    ...
                                    <div class="small text-muted">{{ ucfirst((string)($it['type'] ?? 'service')) }} · {{ $qty }} × {{ $fmt($price) }}</div>
                                    @if($startsAt !== '')
                                        <div class="small text-muted">Schedule: {{ $startsAt }}</div>
                                    @endif
                                </div>
