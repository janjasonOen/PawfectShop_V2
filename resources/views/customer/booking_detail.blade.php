...
<div class="container py-4">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                ...
                <h3 class="section-title mb-0">Booking #{{ (int)$bookingId }}</h3>
            </div>
            <div class="text-muted">Detail booking service dan ringkasan pembayaran.</div>
        </div>
        <div class="d-flex gap-2">
            <a class="btn btn-outline-secondary" href="{{ route('customer.bookings') }}"><i class="bi bi-arrow-left me-1"></i>Kembali</a>
    ...
    @php
        $canUpload = strtolower((string) ($booking->status ?? '')) === 'pending';
    @endphp
    ...
    <div class="row g-4">
                ...
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                        <div>
                            ...
                            <div class="text-muted small">Cek service, qty, dan jadwal.</div>
                        </div>
                        <div class="text-end">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start gap-3">
                                        <div class="d-flex align-items-start gap-3">
                                            ...
                                            <div class="text-muted small">
                                                {{ ucfirst((string)($it['type'] ?? 'service')) }} · Qty: {{ $qty }} · {{ $fmt($price) }}
                                            </div>
                                            @if($startsAt !== '')
                                                <div class="text-muted small mt-1"><i class="bi bi-calendar-event me-1"></i>Schedule: {{ $startsAt }}</div>
                                            @endif
                                            </div>
                                        </div>
        ...
        <div class="col-lg-4">
            <div class="d-flex flex-column gap-3">
                <div class="card shadow-sm">
                    <div class="card-body p-4">
                        <div class="fw-semibold mb-2">Booking Summary</div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="fw-semibold">Total</span>
                            <span class="fw-bold fs-5">{{ $fmt($total) }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        @if(!empty($booking->booking_date))
                            <div class="d-flex align-items-center justify-content-between mt-2">
                                ...
                                <span class="small">{{ $booking->booking_date }}</span>
                            </div>
                        @endif
                        @if(!empty($booking->service_datetime))
                            <div class="d-flex align-items-center justify-content-between mt-2">
                                <span class="text-muted">Service</span>
                                <span class="small">{{ $booking->service_datetime }}</span>
                            </div>
                        @endif
                        <div class="d-flex align-items-center justify-content-between mt-2">
                ...
                <div class="card shadow-sm" id="payment">
                    <div class="card-body p-4">
                        ...
                        @if($paymentProofPath !== '')
                            ...
                            @if(!$canUpload)
                                <div class="text-muted small">Upload ulang tidak tersedia karena status sudah {{ ucfirst((string) ($booking->status ?? '-')) }}.</div>
                            @endif
                        @else
                        ...
                        @else
                            @if($paymentProofPath === '')
                                <div class="text-muted small">Upload bukti pembayaran tidak tersedia karena status sudah {{ ucfirst((string) ($booking->status ?? '-')) }}.</div>
                            @endif
                        @endif
                    </div>
                </div>
                ...
                <div class="card shadow-sm">
                    <div class="card-body p-4">
                        ...
                        <div class="small text-muted">{{ $booking->customer_name ?? '' }}</div>
                        <div class="small text-muted">{{ $booking->customer_email ?? '' }}</div>
                        <div class="small text-muted">{{ $booking->customer_phone ?? '' }}</div>
                        <div class="mt-2 small" style="white-space:pre-line;">{{ $booking->customer_address ?? '' }}</div>
                    </div>
                </div>
            </div>
        </div>
